# 👋 Hi, I'm Nibin M

💼 **Project Engineer @ Wipro**  
🔧 Full-Stack Developer | ServiceNow Specialist | Cloud Certified (AWS & Azure)  
🚀 Passionate about building scalable web apps, automating workflows, and solving real-world problems.

---

### 🛠️ Tech Stack
- **Languages & Frameworks**: C#, .NET, Angular, JavaScript, HTML/CSS
- **Platforms & Tools**: ServiceNow, GitHub Copilot, Visual Studio, Postman
- **Cloud & DevOps**: AWS (Cloud Practitioner), Azure (Fundamentals), Git

---

### 🌟 Featured Projects
- **MoneyManager App** – A Flutter-based mobile app for tracking expenses, budgeting, and financial analysis.  
- **ServiceNow Workflow Automation** – Automated ITSM workflows to reduce resolution time and improve efficiency.  
- **Angular/.NET Web App** – Full-stack application showcasing modular architecture and responsive UI.

---

### 📚 Certifications
- AWS Certified Cloud Practitioner  
- Microsoft Certified: Azure Fundamentals

---

### 📫 Connect with Me
- [LinkedIn](https://www.linkedin.com/in/itsnibin/)
- [GitHub](https://github.com/ItsNibin)
- [Portfolio](https://nibin-portfolio.netlify.app)
